...
class AddressParser(object):
    ...
    def Check_Reverse_Key(self, s):
        ...
        pattern_house = re.search(
            '(ka)*[a-z]*\d*/*[a-z]*(ka)*\d+\s+(no|number)\s+house\s+', s)
        pattern_road = re.search(
            '(ka)*[a-z]*\d*/*[a-z]*(ka)*\d+\s+(no|number)\s+road\s+', s)
        pattern_goli = re.search(
            '(ka)*[a-z]*\d*/*[a-z]*(ka)*\d+\s+(no|number)\s+goli\s+', s)
        pattern_lane = re.search(
            '(ka)*[a-z]*\d*/*[a-z]*(ka)*\d+\s+(no|number)\s+lane\s+', s)
        pattern_block = re.search(
            '(ka)*[a-z]*\d*/*[a-z]*(ka)*\d+\s+(no|number)\s+block\s+', s)
        pattern_sector = re.search(
            '(ka)*[a-z]*\d*/*[a-z]*(ka)*\d+\s+(no|number)\s+sector\s+', s)
        if pattern_house:
