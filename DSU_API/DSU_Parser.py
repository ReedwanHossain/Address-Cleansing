...
from DSU_API import preprocessor
from DSU_API import synonym
from DSU_API import word_preprocessing
import json
import time
...
area_as_ds=['Adabor','Badda','Bangshal','Kotwali','Bijoynagar','Biman Bandar','Demra','Dhanmondi','Dohar','Fulbaria','Gendaria','Gulshan','Harirampur','Hazaribagh','Islampur','Jagannathpur','Jatrabari','Kadamtali','Kafrul','Kalabagan','Kaliganj','Kaliganj','Kaliganj','Kaliganj','Keraniganj','Khilgaon','Khilkhet','Kotwali','Kotwali','Kotwali','Lalbagh','Manda','Mirpur','Mirpur','Mohammadpur','Mohammadpur','Mohanpur','Motijheel','Nabinagar','Nawabganj','Nawabganj','New Market','Pallabi','Paltan','Rajpara','Ramna','Rampura','Sabujbagh','Savar','Shahbagh','Shahjadpur','Shyampur','Sutrapur','Tejgaon','Turag','Uttara']
...
def Identifier2(input_address):
  ...
  for district,area_info in DSU.items(): 
    ...
    if find_union!="None" or (find_subdistrict!="None" and len(find_subdistrict)!=0):
        if find_subdistrict not in area_as_ds:
          result_dictionary["district"]=district
  ...
  if len(res)==0: 
      ...
      if len(ans1)>=2:
        ...
        for j in range(0,len(ans1)):
          find=True
          try:
            if ans1[j]['sub_district'] in district:
              find=False
          except Exception:
            pass 
          for x in Type:
            if (ans1[j]['sub_district']+' '+x).lower() in input_address.lower():
              find=False
              break
            elif finalresult((ans1[j]['sub_district']+' '+x),input_address)==True:
              find=False
              break
      ...
      if len(res)>=2:
        ...
        for j in range(0,len(res)):
          ...
          for x in Type:
            if (res[j]['district']+' '+x).lower() in input_address.lower() or (res[j]['sub_district']+' '+x).lower() in input_address.lower():
              find=False
              break
            elif finalresult((res[j]['sub_district']+' '+x),input_address)==True or finalresult((res[j]['district']+' '+x),input_address)==True:
              find=False
              break
        ...
        if len(temp_result)>=2:
            ...
            for i in range(0,len(temp_result)):
                if finalresult(temp_result[i]['sub_district'],input_address)==True:
                    temp_result2.append(temp_result[i])
            if len(temp_result2)==1:
                not_parse=temp_result2
            elif len(temp_result2)>=2:
                not_parse.append(temp_result2[0])
            elif len(temp_result2)==0:
                if temp_result[0]['district']!=temp_result[0]['union']:
                    not_parse.append(temp_result[0])
                else:
                    not_parse.append(temp_result[1])
        else:
            not_parse.append(temp_result)
        return not_parse
      else:
        #print(res)
        parse=res
...
def DSU_Parser(input_address):
  parse_list=[]
  parse_dictionary={'district':None, 'sub_district':None, 'union':None}
  parse=Identifier(input_address)
  if len(parse)==0:
    parse_list.append(parse_dictionary)
  else:
    if parse[0]['district']!='None':
      parse_dictionary['district']= parse[0]['district']
    if parse[0]['sub_district']!='None':
      parse_dictionary['sub_district']= parse[0]['sub_district']
    if parse[0]['union']!='None':
      parse_dictionary['union']= parse[0]['union']
    parse_list.append(parse_dictionary)
  return parse_list
