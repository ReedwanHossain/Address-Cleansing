      ...
      if len(res)>=2:
        ...
        if len(temp_result)>=2:
            ...
            for i in range(0,len(temp_result)):
                if finalresult(temp_result[i]['Subdistrict'],input_address)==True:
                    temp_result2.append(temp_result[i])
            if len(temp_result2)==1:
                not_parse['Predicted Result']=temp_result2
            elif len(temp_result2)>=2:
                not_parse['Predicted Result']=temp_result2[0]
            elif len(temp_result2)==0:
                if temp_result[0]['District']!=temp_result[0]['Union']:
                    not_parse['Predicted Result']=temp_result[0]
                else:
                    not_parse['Predicted Result']=temp_result[1]
        else:
            not_parse['Predicted Result']=temp_result
        return not_parse
      else:
